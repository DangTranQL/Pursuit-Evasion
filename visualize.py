...
def train_agent(maze, pursuer, pursuer2, evader, q_learning, q_learning2, q_learning_avoid, max_steps=300):
    ...
    for step in range(max_steps):
        if pursuer.position == evader.position or pursuer2.position == evader.position:
            ...
            evader_action = q_learning_avoid.choose_action(evader_state)
...
# Testing the performance of the agents
def testing(maze, pursuer, pursuer2, evader, q_learning, q_learning2, q_learning_avoid, max_steps=300):
    for step in range(max_steps):
        if pursuer.position == evader.position or pursuer2.position == evader.position:
            print("Caught the evader!")
            print("Step: ", step)
